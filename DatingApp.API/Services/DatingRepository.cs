    ...
    public class DatingRepository : IDatingRepository {
        ...
        public async Task<Like> GetLike (int userId, int receipientId) {
            return await _context.Likes
                .FirstOrDefaultAsync (u => u.LikerId == userId && u.LikeeId == receipientId);
        }
        ...
        public async Task<PagedList<Message>> GetMessageForUser (MessageParams messageParams) {
            ...
            switch (messageParams.MessageContainer) {
                case "Inbox":
                    messages = messages.Where (u => u.ReceipientId == messageParams.UserId);
                    break;
                case "Outbox":
        ...
        public async Task<IEnumerable<Message>> GetMessageThread (int userId, int receipientId) {
            ...
            var messages = await  _context.Messages.Include (u => u.Sender).ThenInclude (p => p.Photos)
                .Include (u => u.Receipient).ThenInclude (p => p.Photos)
                .Where(m => m.ReceipientId == userId && m.SenderId == receipientId 
                        || m.ReceipientId == receipientId && m.SenderId == userId)
